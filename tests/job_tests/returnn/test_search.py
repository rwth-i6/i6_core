...
def test_search_bpe_to_words_single():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        bpe_to_words_job.out_word_search_results = Path(os.path.join(tmpdir, "word_search_results_single.py"))
        bpe_to_words_job.run()

        reference_dict = eval(open(reference_word_search_results.get_path(), "rt").read())
        job_dict = eval(open(bpe_to_words_job.out_word_search_results.get_path(), "rt").read())
...
def test_search_bpe_to_words_nbest():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        bpe_to_words_job = SearchBPEtoWordsJob(search_out)
        bpe_to_words_job.out_word_search_results = Path(os.path.join(tmpdir, "word_search_results_nbest.py"))
        bpe_to_words_job.run()
