...
def test_sclite_job():
  with tempfile.TemporaryDirectory() as tmpdir:
    ...
    assert sclite_job.out_num_errors.get() == 15, "Wrong num errors, %s instead of 15" % str(sclite_job.out_num_errors.get())
    assert sclite_job.out_percent_correct.get() == 36.4, "Wrong percent correct, %s instead of 36.4" % str(sclite_job.out_percent_correct.get())
    assert sclite_job.out_num_correct.get() == 8, "Wrong num correct, %s instead of 8" % str(
      sclite_job.out_num_correct.get())
    assert sclite_job.out_percent_substitution.get() == 40.9, "Wrong percent substitution, %s instead of 40.9" % str(
      sclite_job.out_percent_substitution.get())
    ...
    assert sclite_job.out_percent_word_accuracy.get() == 31.8, "Wrong percent word accuracy, %s instead of 31.8" % str(
      sclite_job.out_percent_word_accuracy.get())
